...
class CollabRepositoryImpl(private val database: FirebaseDatabase) : CollabRepository {
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (boardId == null) {
                    ...
                    return@withContext dev.borisochieng.firebase.auth.data.FirebaseResponse.Error("Failed to generate board id")
                }
                ...
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Success(boardDetails)
            } catch (e: Exception) {
                e.printStackTrace()
                Log.e("Create sketch", e.message.toString())
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Error("Something went wrong please try again")
            }

        }

    override suspend fun fetchExistingSketches(userId: String): dev.borisochieng.firebase.auth.data.FirebaseResponse<List<Sketch>> =
        withContext(Dispatchers.IO) {
            ...
            return@withContext try {
                ...
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Success(domainSketches)
            } catch (e: Exception) {
                e.printStackTrace()
                Log.e("Fetch sketches", e.message.toString())
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Error("Cannot fetch sketches, check your internet connection and try again")
    ...
    ): dev.borisochieng.firebase.auth.data.FirebaseResponse<String> =
        withContext(Dispatchers.IO) {
            val pathRef =
                databaseRef.child("Users")
                    .child(userId)
            ...
            return@withContext try {
                ...
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Success("Sketch updated")

            } catch (e: Exception) {
                e.printStackTrace()
                Log.e("Update path", e.message.toString())
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Error("Error syncing sketches")
            }
    ...
    override suspend fun listenForPathChanges(
        userId: String,
        boardId: String
    ): Flow<dev.borisochieng.firebase.auth.data.FirebaseResponse<List<PathProperties>>> =
        callbackFlow {
            val boardRef =
                databaseRef
                    .child("Users")
            ...
            val listener = object : ChildEventListener {
                ...
                override fun onChildAdded(snapshot: DataSnapshot, previousChildName: String?) {
                    ...
                    deserializedNewPath?.let {
                        //emit new paths
                        trySend(dev.borisochieng.firebase.auth.data.FirebaseResponse.Success(listOf(it.toPathProperties())))
                    }
                }
...
//                    }
                ...
                override fun onCancelled(error: DatabaseError) {
                    ...
                    trySend(dev.borisochieng.firebase.auth.data.FirebaseResponse.Error("Failed to fetch latest path: ${error.message}"))
                }
    ...
    override suspend fun deleteSketch(userId: String, boardId: String): dev.borisochieng.firebase.auth.data.FirebaseResponse<String> =
        withContext(Dispatchers.IO) {
            val boardRef = databaseRef.child("Users").child(userId).child("boards").child(boardId)

            return@withContext try {
                boardRef.removeValue().await()
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Success("Sketch deleted")
            } catch (e: Exception) {
                e.printStackTrace()
                Log.e("Deleted Board", e.message.toString())
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Error("Failed to delete board, please try again.")
            }
        }
    ...
    override suspend fun renameSketchInRemoteDB(
        ...
        withContext(Dispatchers.IO) {
            ...
            return@withContext try {
                ...
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Success("Sketch renamed successfully")

            } catch (e: Exception) {
                e.printStackTrace()
                Log.e("Rename sketch", e.message.toString())
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Error("Cannot rename sketch, please try again")
            }
        }
    ...
    override suspend fun fetchSingleSketch(
        userId: String,
        boardId: String
    ): dev.borisochieng.firebase.auth.data.FirebaseResponse<Sketch> =
        withContext(Dispatchers.IO) {
            try {
                val boardRef =
                ...
                if (dbSketch != null) {
                    ...
                    dev.borisochieng.firebase.auth.data.FirebaseResponse.Success(deserializedDBSketch?.toSketch())
                } else {
                    dev.borisochieng.firebase.auth.data.FirebaseResponse.Error("Canvas not found")
                }
                ...
                dev.borisochieng.firebase.auth.data.FirebaseResponse.Error("Failed to fetch sketch")
