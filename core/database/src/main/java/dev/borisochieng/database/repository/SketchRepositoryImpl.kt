...
import dev.borisochieng.model.MessageType
import dev.borisochieng.model.Sketch
import dev.borisochieng.database.SketchDao
...
class SketchRepositoryImpl: SketchRepository, KoinComponent {
	...
	override fun getAllSketches(): Flow<List<dev.borisochieng.model.Sketch>> {
		return sketchDao.getAllSketches()
	}

	override fun getSketch(sketchId: String): Flow<dev.borisochieng.model.Sketch> {
		return sketchDao.getSketch(sketchId)
	}

	override suspend fun saveSketch(sketch: dev.borisochieng.model.Sketch) {
		return sketchDao.saveSketch(sketch)
	}

	override suspend fun refreshDatabase(sketches: List<dev.borisochieng.model.Sketch>) {
		sketchDao.clearDatabase()
		return sketchDao.insertSketches(sketches)
	}

	override suspend fun updateSketch(sketch: dev.borisochieng.model.Sketch) {
		return sketchDao.updateSketch(sketch)
	}

	override suspend fun deleteSketch(sketch: dev.borisochieng.model.Sketch) {
	...
	override suspend fun createChats(message1: String,boardId: String): Flow<Boolean> {
		...
		val message = dev.borisochieng.model.MessageModel(
            ...
            audioUrl = null,
            messageType = dev.borisochieng.model.MessageType.MESSAGE,
	...
	override suspend fun getChats(boardId: String): Flow<List<dev.borisochieng.model.MessageModel>> {
		val messages = mutableStateListOf<dev.borisochieng.model.MessageModel?>()
			...
			firestore.collection("projects")
				...
				.addOnSuccessListener { querySnapshot ->
					for (document in querySnapshot.documents) {
						val model = document.toObject<dev.borisochieng.model.MessageModel>()
						messages.add(model)
					}
	...
	override suspend fun loadChats(boardId: String): Flow<List<dev.borisochieng.model.MessageModel>> {
		val messages = mutableStateListOf<dev.borisochieng.model.MessageModel?>()
				...
				.addSnapshotListener { querySnapshot, exception ->
					...
					if (querySnapshot != null) {
						...
						for (documentSnapshot in querySnapshot.documents) {
							...
							val messageModel = documentSnapshot.toObject(dev.borisochieng.model.MessageModel::class.java)
